...
class TestGeolocalizationViewPOSTTestCase(TestCase):
    def setUp(self):
        self.client.post(
            "/api/register/", {"username": "test", "password": "test"}, format="json"
        )

    def test_add_new_ip_unlogged_user(self):
        response = self.client.post(
            "/api/geolocalization/", {"address": "12.12.12.12"}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_add_new_correct_ip_logged_user(self):
        login_response = self.client.post(
            "/api/token/", {"username": "test", "password": "test"}, format="json"
        )
        ...
        self.assertEqual(
            response.data["message"], f"IP {ip} was saved to the database."
        )
        ...
        self.assertEqual(
            ...
            f"Geolocalization with this ip ({ip}) already exists in the database.",
        )
    ...
    def test_add_new_incorrect_ip_logged_user(self):
        ...
        self.assertEqual(
            response.data["message"],
            "[Errno -2] Name or service not known. Please provide a valid IP address in body parameters.",
        )
        db = Geolocalization.objects.filter(ip=ip)
...
class TestGeolocalizationViewGETTestCase(TestCase):
    ...
    def test_get_incorrect_ip_logged_user(self):
        ...
        self.assertEqual(
            response.data["message"],
            "Geolocalization matching query does not exist. Please provide a valid IP address in body parameters.",
        )
        db = Geolocalization.objects.filter(ip=ip)
        self.assertEqual(len(db), 0)
...
class TestGeolocalizationViewDELETETestCase(TestCase):
    ...
    def test_delete_correct_ip_logged_user(self):
        ...
        self.assertEqual(db.ip, ip)
        response = self.client.delete(
            f"/api/geolocalization/?address={ip}",
            content_type="application/json",
            HTTP_AUTHORIZATION=token,
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response.data["message"], "Geolocalization for IP 12.12.12.12 was removed."
        )
        db = Geolocalization.objects.filter(ip=ip)
        self.assertEqual(len(db), 0)
